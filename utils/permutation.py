...
Derived from module permutation.py, v0.1.0. That module uses a "dummy index 0".
...
N0 denotes the set of non negative integers 0,1,2,...

The permutation (2,0,1) and (2,0,1,3,4) are considered equal. More generally:
A permutation is considered as a function f:N0-->N0 with the following 2 properties:
  (1) one-to-one
  (1) only a finite number of elements of N0 are changed by f
...
class Permutation:
    ...
    def _normalize(self):
        ...
        e.g. (2,0,1,3,4) is reduced to (2,0,1)
        """
        n = len(self.p)-1
    ...
    def length(self):
        """ Returns the length of the internal list representing the permutation.
    ...
    def apply(self, a_list):
        """ Applies the permutation to a_list and returns the new list.
    ...
    @classmethod
    def sorted(cls, a_list, key=None, reverse=False):
        """
        Returns a sorted copy of a_list and the permutation that when applied to a_list returns this copy.
